...
class AdvancedPropagationEngine:
    ...
    def calculate_path_loss(
        ...
        shadowing = np.random.normal(0, model.shadowing_std_db)

        total_loss = fspl_db + distance_factor + shadowing
        if model.rayleigh_fading_enabled:
            total_loss += np.random.rayleigh(scale=2.0)
        return total_loss
    ...
    def optimize_multi_band_selection(
        ...
        *,
        ...
        for band in available_bands:
            ...
            band_scores.append((band, total_score))
...
def demo() -> str:
    ...
    np.random.seed(7)
    engine = AdvancedPropagationEngine()
    ...
    for _ in range(20):
        engine.link_history[("node_a", "node_b")] = deque(
            [20 + np.random.normal(0, 2) for _ in range(20)], maxlen=50
        )
