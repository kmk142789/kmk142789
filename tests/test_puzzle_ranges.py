...
def test_summarise_puzzle71_ranges() -> None:
    ...
    assert summary.count == 121
    assert summary.range_min == int("400da77fc000000000", 16)
    ...
    assert summary.total_keys == 121 * (1 << 36)
    assert summary.coverage_ratio == pytest.approx(7.043126970529556e-09)

    report = format_summary(summary)
    assert "Total ranges: 121" in report
