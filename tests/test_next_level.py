...
def _markers(template: str) -> str:
    """Return ``template`` with placeholder markers expanded.

    The helper prevents this module from contributing TODO/FIXME entries to the
    real ``ROADMAP.md`` output while still letting the tests exercise the
    scanner with realistic comment text.
    """

    return template.replace("<<TODO>>", "TODO").replace("<<FIXME>>", "FIXME")
...
def test_discover_tasks_skips_common_virtualenvs(tmp_path):
    ...
    skipped.write_text(_markers("# <<TODO>> inside virtualenv\n"), encoding="utf-8")

    included = tmp_path / "app.py"
    included.write_text(_markers("# <<TODO>> keep me\n"), encoding="utf-8")
...
def test_discover_tasks_respects_max_file_size(tmp_path):
    ...
    large_payload = _markers("# <<TODO>> expansive refactor\n") + ("x" * 2048)
...
def test_discover_tasks_supports_nested_skip_paths(tmp_path):
    ...
    skipped.write_text(_markers("# <<TODO>> hidden\n"), encoding="utf-8")

    included = tmp_path / "src" / "other.py"
    included.write_text(_markers("# <<TODO>> keep\n"), encoding="utf-8")
...
def test_discover_tasks_handles_platform_specific_paths(tmp_path):
    ...
    nested.write_text(_markers("# <<TODO>> hidden\n"), encoding="utf-8")

    keep = tmp_path / "src" / "module_two.py"
    keep.write_text(_markers("# <<TODO>> keep\n"), encoding="utf-8")
...
def test_discover_tasks_filters_extensions(tmp_path):
    ...
    py_task.write_text(_markers("# <<TODO>> keep python\n"), encoding="utf-8")

    txt_task = tmp_path / "notes.txt"
    txt_task.write_text(_markers("# <<TODO>> keep text\n"), encoding="utf-8")
...
def test_discover_tasks_respects_ignore_patterns(tmp_path):
    ...
    ignored_dir.write_text(_markers("# <<TODO>> hidden\n"), encoding="utf-8")

    ignored_file = tmp_path / "module.generated.py"
    ignored_file.write_text(_markers("# <<TODO>> skip file\n"), encoding="utf-8")
...
def test_discover_tasks_trims_comment_text(tmp_path):
    ...
    inline.write_text(_markers("/* <<TODO>> align portal */\n"), encoding="utf-8")
...
def test_summarise_tasks_returns_sorted_counts(tmp_path):
    ...
    first.write_text(_markers("# <<TODO>> first\n"), encoding="utf-8")

    second = tmp_path / "docs" / "guide.md"
    second.parent.mkdir()
    second.write_text(_markers("<!-- <<FIXME>> another -->\n"), encoding="utf-8")
...
def test_write_json_report_persists_payload(tmp_path):
    source = tmp_path / "module.py"
    source.write_text(_markers("# <<TODO>> persist\n"), encoding="utf-8")
