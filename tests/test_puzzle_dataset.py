...
def test_generate_private_key_sequence_matches_expected_range():
    expected = [
    ...
    generated = generate_private_key_sequence(0x88, 0xB4)
    assert generated == expected
    assert len(generated) == 0xB4 - 0x88 + 1


def test_generate_private_key_sequence_supports_custom_width():
    assert generate_private_key_sequence(0x88, 0x8A, width=4) == [
        "0088",
        "0089",
        "008a",
    ]
...
def test_generate_private_key_sequence_validates_arguments():
    with pytest.raises(ValueError):
        generate_private_key_sequence(5, 4)
    with pytest.raises(ValueError):
        generate_private_key_sequence(4, 5, width=0)
