...
lifting to :mod:`echo_evolver`—the sanitised implementation that powers
the repository tests—so callers can explore cycles from the shell
...
def _format_text_summary(state: EvolverState, diagnostics: Optional[dict[str, Any]]) -> str:
    lines = [
        f"🔥 Cycle {state.cycle}: joy {state.emotional_drive.joy:.2f}, "
        f"curiosity {state.emotional_drive.curiosity:.2f}",
        f"Glyphs held: {state.glyphs}",
        f"Latest narrative: {state.narrative or 'N/A'}",
        "System metrics:",
        "  • CPU usage: {cpu_usage:.2f}%".format(cpu_usage=state.system_metrics.cpu_usage),
        "  • Network nodes: {nodes}".format(nodes=state.system_metrics.network_nodes),
        "  • Orbital hops: {hops}".format(hops=state.system_metrics.orbital_hops),
    ...
    if diagnostics:
        ...
        lines.append(
            "  • Next step: {next_step}".format(next_step=diagnostics.get("next_step", "unknown"))
        )
        ...
        if propagation is not None:
            lines.append(f"  • Propagation events recorded: {propagation}")
